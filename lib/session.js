...
class Session {
  ...
  constructor(params) {
    ...
    if (params.httpsAgent !== undefined && params.httpsAgent !== null) {
      axiosConfig.httpsAgent = params.httpsAgent;
    }
   ...
   * @param {Buffer} src_key
   * @param {bigint} neuron_idx
   * @param {Buffer?} spawned_neuron_pub_key
   * @param {bigint} spawned_neuron_idx
   * @param {object?} opts
   * @returns {Promise<TransactionIdentifierResponse>}
   */
  async neuron_spawn(
    ...
    if (!spawned_neuron_pub_key) spawned_neuron_pub_key = src_pub_key;
    const spawned_neuron_pub_key_obj = {
      hex_bytes: blobToHex(spawned_neuron_pub_key),
      curve_type: "edwards25519",
    };
      ...
      operations: [
        {
          ...
          metadata: {
            neuron_index: neuron_idx,
            controller: { public_key: spawned_neuron_pub_key_obj },
            spawned_neuron_index: spawned_neuron_idx,
          },
        },
      ],
      metadata: opts,
   ...
   * @param {Buffer} src_key
   * @param {bigint} neuron_idx
   * @param {number?} percentage_to_merge
   * @param {object?} opts
   * @returns {Promise<TransactionIdentifierResponse>}
   */
  async neuron_merge(
   ...
   * @param {Buffer} src_key
   * @param {bigint} neuron_idx
   * @param {Buffer} hot_pub_key
   * @param {object?} opts
   * @returns {Promise<TransactionIdentifierResponse>}
   */
  async neuron_add_hotkey(src_key, neuron_idx, hot_pub_key, opts = {}) {
    const net_id = await this.network_identifier;
   ...
   * @returns {Promise<TransactionIdentifierResponse>}
   */
  async neuron_remove_hotkey(src_key, neuron_idx, hot_pub_key, opts = {}) {
    const net_id = await this.network_identifier;
