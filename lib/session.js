...
class Session extends RosettaClient {
  ...
  async transactions(tx_hash_hex) {
    const res = await axios.post(`${this._baseUrl}/search/transactions`, {
      network_identifier: await this.network_identifier,
      transaction_identifier: { hash: tx_hash_hex },
    });
    return res.data;
  }
}
